...
async function run() {
  // 1. Create App Store Version 1.6.1
  console.log('1. Creating App Store Version 1.6.1...');
  ...
  // 2. Update localization (whatsNew for 1.6.1)
  ...
  await api('PATCH', `/v1/appStoreVersionLocalizations/${locId}`, {
    data: {
      ...
      id: locId,
      attributes: {
        whatsNew: '1.6.1 更新内容：\n- 多语言国际化支持：全端支持简体中文、English、日本語无缝切换\n- 从App界面到分享长图再到ESP32硬件计分板全部支持多语言\n- 修复启动白屏问题'
      }
    }
  });
  ...
  console.log('\n3. Waiting for build 1.6.1 to finish processing...');
  let buildId = null;
  while (!buildId) {
  ...
  console.log('\n🎉 v1.6.1 SUBMITTED TO APP STORE FOR REVIEW!');
