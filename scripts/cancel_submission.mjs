...
async function run() {
  const subs = await api('GET', `/v1/apps/${appId}/reviewSubmissions?filter[state]=WAITING_FOR_REVIEW`);
  if (subs.data && subs.data.length > 0) {
    for (const sub of subs.data) {
      console.log(`Canceling submission ${sub.id}...`);
    ...
    console.log('No submissions in WAITING_FOR_REVIEW found.');
  }
}
